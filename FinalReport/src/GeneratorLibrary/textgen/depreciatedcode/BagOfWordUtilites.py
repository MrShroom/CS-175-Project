...
def getReviews(numOfReviews, starRating):
    statement = "SELECT review_text FROM reviews WHERE reviews.stars="+str(starRating)+" LIMIT "+str(numOfReviews)+";"
    cnx = mysql.connector.connect(**config)
    cursor = cnx.cursor()
    cursor.execute(statement)
    output = []
